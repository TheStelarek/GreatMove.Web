...
import styles from '@/components/trainingPlan/searchExercise/SearchExercise.module.scss';
import { useAppDispatch } from '@/store/hooks/useAppDispatch';
import { addExercise } from '@/store/trainingPlan/TrainingPlanSlice';
...
const SearchExercise: FC<SearchExerciseProps> = ({ setShowInput, trainingDayId }) => {
   const dispatch = useAppDispatch();
   const [matchingExercises, setMatchingExercises] = useState<Array<any>>([]);
   ...
   const getMatchingExercises = debounce(async ({ inputValue }) => {
      ...
      const response = await apiClient.get(`/recipes?take=8&skip=0&name=${inputValue}`);
